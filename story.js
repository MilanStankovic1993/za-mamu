...
const slides=[...notes.map(x=>({type:"note",...x})),...Array.from({length:27},(_,i)=>({type:"photo",src:`uspomene/uspomena-${String(i+1).padStart(2,"0")}.webp`,number:i+1})),{type:"video"},{type:"final"}];
...
function render(){
 ...
 if(s.type==="video")content=`<div class="video-slide"><p class="eyebrow">Pojačaj zvuk</p><h2>Još jedna poruka za tebe.</h2><video id="gift-video" controls playsinline preload="metadata" src="zavrsni-snimak-1.mp4"></video><small>Kada se snimak završi, čeka te poslednja poruka.</small></div>`;
 ...
 const video=document.querySelector("#gift-video");if(video){video.onplay=()=>{song.pause();playing=false;paused=true};video.onended=()=>{current=slides.length-1;render()}}
 schedule();
}
